...
def _product(id, name, description=""):
    return {"id": id, "name": name, "description": description, "price": 100.0, "currency": "INR", "availability": "in_stock"}


def test_filter_relevant_products_keeps_only_keyword_matches():
    products = [
        _product("1", "Green Striped T-Shirt", "100% cotton, green stripes"),
        _product("2", "Bangalore Watch Co Weekender", "A stainless steel watch"),
        _product("3", "Lavender Room Spray", "A calming lavender scent"),
    ]

    result = buyer_mod._filter_relevant_products("a green striped t-shirt", products)

    ids = {p["id"] for p in result}
    assert ids == {"1"}


def test_filter_relevant_products_ranks_by_match_count():
    products = [
        _product("1", "Green Shirt", "A plain green shirt"),
        _product("2", "Green Striped T-Shirt", "100% cotton, green stripes, t-shirt"),
    ]

    result = buyer_mod._filter_relevant_products("a green striped t-shirt", products)

    # product 2 shares more goal keywords ("green", "striped", "shirt") than product 1
    # ("green", "shirt") -- must rank first.
    assert [p["id"] for p in result] == ["2", "1"]


def test_filter_relevant_products_caps_to_limit():
    products = [_product(str(i), f"Blue Widget {i}", "A blue widget") for i in range(10)]

    result = buyer_mod._filter_relevant_products("blue widget", products, limit=3)

    assert len(result) == 3


def test_filter_relevant_products_falls_back_to_full_catalog_on_no_matches():
    products = [
        _product("1", "Green Striped T-Shirt", "100% cotton"),
        _product("2", "Bangalore Watch Co Weekender", "A stainless steel watch"),
    ]

    result = buyer_mod._filter_relevant_products("recommend something nice", products)

    assert result is products or {p["id"] for p in result} == {"1", "2"}


def test_filter_relevant_products_is_case_insensitive():
    products = [_product("1", "GREEN Striped T-Shirt", "cotton")]

    result = buyer_mod._filter_relevant_products("a green shirt", products)

    assert [p["id"] for p in result] == ["1"]


def test_filter_relevant_products_ignores_stopwords():
    # "a", "the", "for", "good" are filler -- must not, on their own, match every product
    # that happens to contain a common word by coincidence.
    products = [
        _product("1", "The Good Umbrella", "Great for a rainy day"),
        _product("2", "Green Striped T-Shirt", "cotton"),
    ]

    result = buyer_mod._filter_relevant_products("a good umbrella for the rain", products)
...
def test_discover_prompt_only_includes_keyword_relevant_products(db_session, merchant):
    tshirt = CatalogItem(
        merchant_id=merchant.id, name="Green Striped T-Shirt", description="100% cotton, green stripes",
        price=500.0, currency="INR", availability="in_stock", agent_readable=True,
    )
    watch = CatalogItem(
        merchant_id=merchant.id, name="Bangalore Watch Co Weekender", description="A stainless steel watch",
        price=26000.0, currency="INR", availability="in_stock", agent_readable=True,
    )
    db_session.add_all([tshirt, watch])
    db_session.flush()
    db_session.add(CatalogManifest(merchant_id=merchant.id, version=1, url="/x", item_ids=[tshirt.id, watch.id]))
    db_session.flush()

    fake_client = MagicMock()
    fake_client.chat.completions.create.return_value = _fake_completion(
        {"status": "match", "selected_item_id": tshirt.id, "reasoning": "Match."}
    )
    ...
    user_message = fake_client.chat.completions.create.call_args.kwargs["messages"][1]["content"]
    assert "Green Striped T-Shirt" in user_message
    assert "Bangalore Watch Co Weekender" not in user_message


def test_discover_prompt_includes_everything_for_a_vague_goal(db_session, merchant):
    tshirt = CatalogItem(
        merchant_id=merchant.id, name="Green Striped T-Shirt", description="100% cotton, green stripes",
        price=500.0, currency="INR", availability="in_stock", agent_readable=True,
    )
    ...
    fake_client = MagicMock()
    fake_client.chat.completions.create.return_value = _fake_completion(
        {"status": "need_more_info", "reasoning": "What are you looking for?"}
    )
    ...
    user_message = fake_client.chat.completions.create.call_args.kwargs["messages"][1]["content"]
    assert "Green Striped T-Shirt" in user_message
    assert "Bangalore Watch Co Weekender" in user_message
